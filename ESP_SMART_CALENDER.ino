...
void Weather_Page_Create (lv_obj_t *parent)     //天气界面控件初始化
{
  ...
  //lv_img_set_zoom(img2, 120);                 //256正常,<256缩小,>256放大,112*112
  lv_img_set_zoom(img2, 96);                    //256正常,<256缩小,>256放大,90*90
  //lv_obj_set_auto_realign(img2, true);
  //lv_obj_set_pos(img2, 0, 126);               //设置的位置 
  lv_obj_align(img2, parent, LV_ALIGN_IN_BOTTOM_RIGHT, -90, -130);//对齐
...
void loop()
{
  ...
  if ((now1 - LastTime1 > 300) && (Page_Id == 0)){//ms
    LastTime1 = now1;
    img_num ++;
    if (img_num > 5){
      img_num = 1;
